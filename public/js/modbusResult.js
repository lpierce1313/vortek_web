...
function intToHex(modbus){
  for(var j = 0; j < modbus.data.length; j++){
    val = parseInt(modbus.data[j]);
...
function hex16ToByte16(modbus){
  var byte_arr = [];
    for(var i = 0; i < modbus.data.length/2; i++){
    var hexOne = parseInt(modbus.data[i].toString().slice(2,4), 16);
    var hexTwo = parseInt(modbus.data[i].toString().slice(4,6), 16);
    byte_arr.push(hexOne);
    byte_arr.push(hexTwo);
  }
  for(var i = 0; i < modbus.data.length; i++){
    modbus.data[i] = byte_arr[i];
  }
}

function byteToAscii(modbus){
  val = '';
  for(var j = 0; j < modbus.data.length - 1; j++){
    var temp = parseInt(modbus.data[j]);
    if(!temp < 32 || !temp > 126 ){
...
function hex16Tohex32(modbus){
  var one = 0, two = 0;
  var hex_arr = [];

  for(var j = 0; j < modbus.input.numReg; j++) { hex_arr[j] = '0X00000000'; }
  var i = 0;
  for(var j = 0; j < modbus.input.numReg - 1; j++){
    one = modbus.data[j].slice(2,6);
    j = j + 1;
    two = modbus.data[j].slice(2,6);
    if(!Number(modbus.input.invert)){
      one = "0X" + two + one;
    }
    else{
      one = "0X" + one + two;
  ...
  for(var j = 0; j < modbus.input.numReg; j++) { modbus.data[j] = hex_arr[j]; }

}

function hex32ToInt32(modbus){
  for(var j = 0; j < modbus.data.length; j++){
      var hex = modbus.data[j].slice(2,10)
      modbus.data[j] = (parseInt(hex, 16));
  }
}

function hex32ToFloat(modbus){
  var byte_arr = [];
  for(var j = 0; j < modbus.data.length; j++){
    var hexOne = parseInt((modbus.data[j].toString().slice(2,4)), 16);
    var hexTwo = parseInt((modbus.data[j].toString().slice(4,6)), 16);
    var hexThree = parseInt((modbus.data[j].toString().slice(6,8)), 16);
    var hexFour = parseInt((modbus.data[j].toString().slice(8,10)), 16);

    var data = [hexOne, hexTwo, hexThree, hexFour];
...
function convertASCII(modbus){
  intToHex(modbus);
  hex16ToByte16(modbus);
  byteToAscii(modbus);
  modbus.result.arr[0] = {regLabel: dataCopy[modbus.keyVal].value, regAdd: dataCopy[modbus.keyVal].key, recType: dataCopy[modbus.keyVal].type, rw: dataCopy[modbus.keyVal].rw, value: modbus.data[0]};
}

function convert16Bit(modbus){
  if(modbus.input.format == 'h'){
    intToHex(modbus);
  }
  else if(modbus.input.format == 'i' || modbus.input.format == 'I'){
  ...
  for(var j = 0; j < modbus.input.numReg; j++){
    modbus.result.arr[j] = {regLabel: dataCopy[modbus.keyVal + j].value, regAdd: dataCopy[modbus.keyVal + j].key, recType: dataCopy[modbus.keyVal + j].type, rw: dataCopy[modbus.keyVal + j].rw, value: modbus.data[j]};
  }
}

function convert32Bit(modbus){

  intToHex(modbus);
  hex16Tohex32(modbus);
  modbus.data.splice(modbus.input.numReg/2, modbus.input.numReg);
  if(modbus.input.format == 'H'){
    //Nothing
  }
  else if(modbus.input.format == 'L' || modbus.input.format == 'l'){
    hex32ToInt32(modbus);
    if(modbus.input.format == 'l'){
      makeSignedInt(modbus,32);
    }
  }
  else{ // 'f' format
    hex32ToFloat(modbus);
...
var methods = {
  generate: function node(modbus){
    ...
    //Switch statement for register generation
